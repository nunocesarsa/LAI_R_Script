...
for (i in 1:nrow(test.csv)){
  ...
  filepath <- list.files(path2out.b.fld)
  
  if (length(filepath)==0){
    print("Magick failed - storing raw image in temp failed imgs folder")
    
    #and we actually download the original failed image to the folder 
    download.file(tgt.url,
                  paste("C:/NeonData/digitalPhotography/temp_failed_imgs/",tgt.unique_id,".NEF",sep=""),method="curl")
    
  } else{
    print("Magick sucessfull")
    ...
    write.csv2(test.csv,"C:/NeonData/digitalPhotography/csv_combined/temp_perimagefile_combined_2019_overstory_failedimages_minimize.csv",row.names = F)
